...
func (im *interfaceMonitor) advertiseInterfaceCreation(index int, pendingInterface *pendingInterface) {
	...
	im.logger.V(1).Info("advertise created", "networking-Iface", newInterface, "index", newInterface.GetIndex())
...
func (im *interfaceMonitor) advertiseInterfaceDeletion(intf networking.Iface) {
	im.logger.V(1).Info("advertise deleted", "networking-Iface", intf, "index", intf.GetIndex())
