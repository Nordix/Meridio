...
// to NSMgr for Connection Monitoring purposes.
...
// 2. No dependency towards the NSM kubernetes namespace is required. (Forwarder NSE local to our service can have
// its URL replaced with inode by NSMgr. Therefore, allow replacing the URL in such cases with preferably with the
// URL of the worker local NSMgr.)
//
// Note: Due to the nature of nsmgr registry server, if registryClient is connected to local NSMgr, then NSMgr
// localBypassNSEServer component will restore the endpoint's URL on Find(). That's because Find()'s main use
// case is to let forwarders lookup NSEs upon NSC connection requests and connect them (either directly or via
// remote NSMgr depending on their location).
	...
	channelReadLoop:
		for ctx.Err() == nil {
			...
			case nse, ok := <-nseChannel:
				...
				nsmgrURL, err := url.Parse(nse.GetNetworkServiceEndpoint().Url)
				if err != nil {
					logger.Error(err, "Failed to parse raw NSMgr URL", "URL", nse.GetNetworkServiceEndpoint().Url)
					continue
				}
				...
				if checkMonitorServer != nil {
					nsmgrURL = checkMonitorServer(nsmgrURL)
				}

				nsmgrAddr := nsmgrURL.String()
				if _, exists := nsmgrs.Load(nsmgrAddr); !exists {
					nsmgrs.Store(nsmgrAddr, true)
					logger.V(1).Info("Extracted NSMgr address", "addr", nsmgrAddr)

					// Start a goroutine for each NSMgr
					go func() {
						logger := logger.WithValues("address", nsmgrAddr)
						defer cleanupManager(logger, nsmgrAddr)
						nsmgrConn, err := grpc.DialContext(
							ctx,
							func() string {
								switch nsmgrURL.Scheme {
								case "tcp":
									return nsmgrURL.Host
								}
								return nsmgrURL.String()
							}(),
							dialOptions...,
						)
						if err != nil {
							logger.Error(err, "Failed to dial NSMgr")
							return
						}
						...
						if err != nil {
							logger.Error(err, "Error from NSMgr MonitorConnectionClient")
							return
						}
						logger.V(1).Info("Connected NSMgr")

						for ctx.Err() == nil {
							event, err := stream.Recv()
							if err != nil {
								logger.Error(err, "Error from NSMgr monitorConnection stream")
...
func cleanupManager(logger logr.Logger, nsmgrAddr string) {
	logger.V(1).Info("Clean up NSMgr", "addr", nsmgrAddr)
